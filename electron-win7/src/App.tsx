...
async function tauriClose() {
  ...
  if (api && api.close) { await api.close(); return; }
  window.close();
}
...
const App: React.FC = () => {
  ...
  const handleMinimize = useCallback(async () => {
    const api = (window as any).electronAPI;
    if (api && api.minimize) await api.minimize();
  }, []);
  ...
  const handleAlwaysOnTop = useCallback(async () => {
    ...
    toggleAlwaysOnTop();
    const api = (window as any).electronAPI;
    if (api && api.setAlwaysOnTop) await api.setAlwaysOnTop(newValue);
  }, [toggleAlwaysOnTop]);
  ...
  return (
    <div style={{
      ...
      {/* §8: brief, non-blocking message for a rejected capturing move */}
      {moveMessage && (
        <div style={{
          position: 'fixed', bottom: 80, left: '50%', transform: 'translateX(-50%)',
          padding: '8px 20px', backgroundColor: 'rgba(180, 60, 30, 0.92)',
          color: '#fff', borderRadius: 6, fontSize: 14, zIndex: 300,
