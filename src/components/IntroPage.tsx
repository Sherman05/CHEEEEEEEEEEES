...
const INTRO_TEXT = `<i>Вводная информация – рекомендуется прочитать перед первым использованием.</i><br><br>
Настоящая программа <b>GI chess-T1</b> представляет собой графический интерфейс тактико-стратегической игры chess-T1, частично похожей на шахматы. Она предоставляет пользователю возможность помощью компьютера играть в chess-T1 с соперником, а также анализировать позиции и просматривать партии. Игра с компьютером (ИИ – искусственным интеллектом) не предусмотрена.<br><br>
Для игры с соперником необходимо, чтобы у каждого из игроков на компьютере была установлена программа, или же игроки используют один и тот же компьютер с программой. Когда игроки пользуются разными компьютерами, они должны самостоятельно вести запись ходов и направлять свои ходы (можно со скриншотами позиций со сделанным ходом) сопернику с помощью мессенджеров или электронной почты. Таким образом воспроизводится классический (но более продвинутый) формат игры «по переписке», широко применявшийся в шахматах в прежние годы.<br><br>
Применение программы предполагает, что пользователь знаком с правилами игры chess-T1. С правилами игры можно ознакомиться с помощью литературы, имеющейся в свободном (бесплатном) доступе в интернете (ключевые слова для поиска: «игра chess-T1»).<br><br>
...
• <b>Анализ</b> — позволяет расставить на доске нужную позицию, и далее кнопкой <b>Ok</b> происходит переход в режим анализа данной позиции с возможностью ходов фигурами<br><br>
...
• <b>Изменить размер</b> — изменение размера окна<br><br>
<b>Индикация номера и очереди хода:</b><br><br>
В интерфейсе программы есть строка индикации номера и очередности хода. Скриншоты позиций сохраняются с именем текста строки индикации, либо пользователь сохраняет, используя другой вариант.<br><br>
...
Посредством команд Меню можно сохранять скриншоты позиций либо в созданную пользователем папку (программа предложит), либо скриншоты будут сохраняться в стандартную папку Изображений.<br><br>
...
Когда белая «пешка-ветеран» вступает на клетки <b>замка</b> черных c8, d8, e8, f8 пользователю (игроку) предлагается на выбор две старшие королевские фигуры для превращения «пешки-ветерана». Аналогично происходит, когда черная «пешка-ветеран» вступает на клетки белых c1, d1, e1, f1.<br><br>
Когда белый Принц вступает на клетки <b>замка</b> черных c8, d8, e8, f8 пользователю (игроку) предлагается на выбор две королевские фигуры: Коннет («Ферзь) и Принц для превращения, или отказа от превращения (выбирая Принца), в соответствии правилом. Аналогично происходит, когда черный Принц вступает на клетки <b>замка</b> белых c1, d1, e1, f1<br><br>
<b>Нотация</b> (самостоятельно)<b>:</b><br><br>
В chess-T1 запись расположения фигур на доске и ходов аналогична шахматной. В качестве сокращений для фигур применяются следующие обозначения:<br><br>
